...
public:
	...
	Cona_Odom(double x_, double y_, double th_)
		: x(x_), y(y_), th(th_){};
	Cona_Odom addMotion(Cona_Odom motion)
	{
	...
	Cona_Odom getMotion(Cona_Odom target)
	{
		
		Cona_Odom motion(target.x - x, target.y - y, target.th - th);
...
public:
	...
	Cona_Odom grid2absol(double col, double row, double th, double col_center, double row_center, double mm2grid)
	{
		double dx = (row_center - row) / mm2grid;
		double dy = (col_center - col) / mm2grid;

		robot_mtx.lock();
		Cona_Odom output = robot.addMotion(Cona_Odom(dx, dy, th));
		robot_mtx.unlock();

		return output;
	}
	Cona_Odom absol2grid(double x, double y, double th, double col_center, double row_center, double mm2grid)
	{
		robot_mtx.lock();
		Cona_Odom motion = robot.getMotion(Cona_Odom(x, y, th));
		robot_mtx.unlock();
		...
		return Cona_Odom(output.x, output.y, th);
	}
	nav_msgs::Odometry cona2ros(Cona_Odom input)
	{
	...
	Cona_Odom ros2cona(nav_msgs::Odometry input)
	{
		Cona_Odom output;
